...
class Zombi:
    def __init__(self, playr, plrUprv, batch=zombiBat, width=35, height=35, col=(50,120,150), type=None, xp=100, speed=1, spawnSpeed=1/2, damage=10):
    ...
    def spawn(self, dt=2, isSpawn=True):      # перенеси функцию спавн в update не делай её отдельно без надобности
        if isSpawn:
            ...
            if r(0,1) == 0:
                ...
                zombies[(sh.Rectangle(coord, coord1, self.width, self.height, self.col, batch=zombiBat))] = (sh.Rectangle(coord, coord1 + self.height, self.width, 4, batch=zombiBat, color=(255, 0, 0)), self.width / self.xp)
            else:
                #print(2,coord1,coord)
                #зомбей справа  и сверху видно не было поэтому я думал что спaвн почему то не работает
                zombies[(sh.Rectangle(coord1, coord, self.width, self.height, self.col, batch=zombiBat))] = (sh.Rectangle(coord1, coord + self.height, self.width, 4, batch=zombiBat, color=(255, 0, 0)), self.width / self.xp)
...
mugs = {}
class Ognestrel:
    def __init__(self, playr, phot="ognestrel.png", damag=10, MaxMugsNum=10, mugsType="common", type=None, isPist=True, bat=bat, mugsNow=100, kd=0.5, mugSpeed=2):
        self.ognTypes={}
