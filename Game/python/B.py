...
playr = Pl()
xp=100
pist = Ognestrel(playr, mugSpeed=30, kd=0.05, mugsNow=100000, MaxMugsNum=1000, damag=100)
...
def on_mouse_press(x,y,button,modifiers):
    print(f"x = {x}, y = {y}")
defaultZomb = Zombi(playr, Uprav, spawnSpeed=1/30)
MiniBoss = Zombi(playr=playr, plrUprv=Uprav, width=70, height=70, type="big", xp=10000, speed=0.2, spawnSpeed=1, damage=50, col=(21, 110, 100))
...
while True:
    ...
    i += 1"""
#defaultZomb.spawn()
#pyglet.clock.schedule_interval(defaultZomb.spawn,defaultZomb.spawnSpeed)
pyglet.clock.schedule_interval(defaultZomb.moving,1/15)
...
pyglet.clock.schedule_interval(MiniBoss.spawn, MiniBoss.spawnSpeed)
pyglet.app.run()